...
در ادامه به بررسی نقطه ای که در فضای Runtime، لینوکس پس دریافت بسته، اجرای روند را به OVS می دهد. برای این منظور به سورس کد لینوکس رفته و با جستجوی تابع \_\_netif\_receive\_skb\_core که در  فایل net/dev/core.h قرار دارد به بررسی مشغول شدیم.

تابع `__netif_receive_skb_core` در هسته لینوکس بخشی از پردازش اصلی داده‌های دریافتی است. این تابع برای پردازش بسته‌های دریافتی و ارسال آن‌ها به لایه‌های بالاتر پروتکل استفاده می‌شود.

این تابع بسته‌های دریافتی را پردازش می‌کند و در صورتی که صف مربوط به پردازنده مربوطه پر باشد و به حداکثر خود رسیده باشد، بسته‌ها را رها می‌کند. برای تنظیم این تنظیمات، ابتدا باید تعیین کنید که آیا نیاز به افزایش backlog وجود دارد یا خیر.

در پروتکل IP به عنوان مثال، تابع `ip_rcv` برای دریافت بسته‌ها استفاده می‌شود. اگر نوع بسته‌ی دریافتی `ETH_P_IP` باشد، تابع `ip_rcv` فراخوانی می‌شود و بسته به ماژول پروتکل IP ارسال می‌شود.



و آنالیز بدنه تابع مربوطه به متغیر rx\_handler رسیدیم که در این جا با استفاده از Switch case مربوطه، بسته را به OVS ارسال می کند. تصویر مربوط به این متغیر را در ذیل قرار دادیم:
