...
<div dir="rtl" style: align="center">
<figure>
  <img src="https://github.com/matinborhani/ovs-tracing/blob/main/screenshots/Ping%20Scenario/perf_call_graph_dp_packet.png" alt="perf ovs_dp_process_packet" />

  *تصویری از perf ovs_dp_packet_packet*
</figure>
</div>
...
ابتدا، ovs_flow_cmd_dump برای بازیابی و نمایش جزئیات جریان‌های فعال در جدول جریان OVS استفاده می‌شود، که شامل قوانین و آمار مربوط به هر جریان است. سپس، ovs_flow_tbl_dump_next به تدریج اطلاعات جریان‌های بعدی را بازیابی می‌کند، که این امر به پیمایش و تجزیه و تحلیل کامل جدول جریان کمک می‌کند. در همین حین، ovs_flow_cmd_fill_info برای فراهم کردن جزئیات دقیق‌تر در مورد جریان‌های خاص به کار گرفته می‌شود، که این اطلاعات برای درک بهتر ساختار و رفتار جریان‌ها حیاتی است. در نهایت، ovs_flow_cmd_fill_stats آمار دقیق مانند تعداد بسته‌ها و بایت‌های عبوری از هر جریان را ارائه می‌دهد، که برای تحلیل کارایی و عملکرد جریان‌ها در شبکه ضروری است. این توابع در کنار هم، تصویری جامع از چگونگی مدیریت ترافیک پینگ در شبکه را فراهم می‌کنند، که برای عیب‌یابی و بهبود شبکه‌های مبتنی بر OVS بسیار مفید است.

## اتصال OVS Module به Kernel

در ادامه به بررسی نقطه ای که در فضای Runtime لینوکس پس دریافت بسته اجرای روند را به OVS می دهد، می پردازیم. برای این منظور به سورس کد لینوکس رفته و با جستجوی تابع `__netif_receive_skb_core` که در  فایل net/core/dev.h قرار دارد به بررسی مشغول می شویم.

تابع __netif_receive_skb_core در ماژول net/core/dev هسته لینوکس یکی از توابع اساسی در سیستم پردازش بسته‌های شبکه است. این تابع بخشی از زیرساخت مدیریت ترافیک شبکه در هسته لینوکس است و نقش حیاتی در دریافت و پردازش بسته‌های شبکه ایفا می‌کند. در اینجا به توضیح مختصری در مورد عملکرد و وظایف این تابع می‌پردازیم: 
 
1. دریافت بسته‌های شبکه: تابع __netif_receive_skb_core مسئول دریافت بسته‌های شبکه (SKB یا Socket Buffer) است که از طریق رابط‌های شبکه به سیستم وارد می‌شوند. 
 
2. پردازش اولیه: این تابع بسته‌های دریافتی را برای پردازش‌های بعدی آماده می‌کند. این شامل بررسی Headerهای بسته و تعیین مسیریابی آن در سیستم است. 
 
3. تعیین نوع پروتکل: تابع مشخص می‌کند که بسته متعلق به کدام پروتکل شبکه است (مانند IP, TCP, UDP و غیره) و بر اساس آن، بسته را به تابع پردازش مربوط به آن پروتکل هدایت می‌کند. 
 
4. فیلترینگ و امنیت: در برخی موارد، __netif_receive_skb_core ممکن است درگیر مکانیزم‌های فیلترینگ شبکه مانند جداول مسیریابی، تنظیمات امنیتی یا فایروال باشد. این به اطمینان از اینکه ترافیک شبکه مطابق با سیاست‌های تعیین شده پردازش می‌شود، کمک می‌کند. 
 
5. تحویل به لایه‌های بالاتر: پس از انجام پردازش‌های اولیه و امنیتی، بسته به لایه‌های بالاتر شبکه مانند لایه Transport (مانند TCP یا UDP) یا حتی لایه کاربردی (مانند HTTP یا FTP) ارسال می‌شود. 
 
6. عملکرد و بهینه‌سازی: __netif_receive_skb_core برای اطمینان از عملکرد بالا و کارایی در پردازش ترافیک شبکه، بهینه‌سازی شده است. این شامل مدیریت حافظه بسته‌ها و کاهش تاخیر در پردازش است. 
 

در زیر برخی از توابع مهمی که معمولاً در داخل __netif_receive_skb_core صدا زده می‌شوند آورده شده‌اند: 
 
1. تابع`skb_checksum_help`: این تابع برای پردازش و تأیید checksum بسته‌ها استفاده می‌شود، که برای اطمینان از صحت داده‌های انتقالی حیاتی است. 
 
2. تابع `ptp_classify_raw`: در صورت استفاده از پروتکل‌های دقیق زمان‌بندی مانند PTP (Precision Time Protocol)، این تابع برای طبقه‌بندی بسته‌ها استفاده می‌شود. 
 
3. تابع `__vlan_hwaccel_rx` و `__vlan_hwaccel_put_tag`: این توابع برای پردازش بسته‌هایی که دارای برچسب VLAN هستند، استفاده می‌شوند. 
 
4. تابع `napi_gro_receive`: برای اجرای عملیات GRO (Generic Receive Offload) که به بهبود عملکرد شبکه کمک می‌کند. 
 
5. تابع`netif_receive_skb` یا `napi_skb_finish`: برای ادامه مسیریابی بسته در سیستم عامل پس از انجام پردازش‌های اولیه. 
 
6. تابع `trace_netif_receive_skb`: برای ردیابی و تشخیص اطلاعات در مورد بسته‌های دریافتی، معمولاً در ابزارهای تحلیل شبکه استفاده می‌شود. 
 
7. تابع `skb_defer_rx_timestamp`: برای زمان‌بندی دقیق بسته‌های دریافتی.


در نهایت و با آنالیز بدنه تابع مربوطه به متغیر rx\_handler رسیدیم که در این جا با استفاده از Switch case مربوطه، بسته را به ماژول OVS ارسال می کند. تصویر مربوط به این متغیر را در ذیل قرار دادیم:
...
|       ovs_vport_send      |  130.612 |  17.538 | 65.980|
...
1. توابع ovs\_vport\_receive و ovs\_dp\_process\_packet: این توابع نیز زمان‌های اجرایی نسبتاً بالایی دارند، به خصوص در مقادیر بیشینه و میانگین. این نشان می‌دهد که دریافت و پردازش بسته‌ها می‌تواند منابع قابل توجهی را مصرف کند. وظایف این توابع در قسمت های پیشین توضیح داده شده است.
  
2. تابع ovs_vport_send: این تابع نیز به علت توضیحات ارائه شده در قسمت های قببل، خود می تواند گلوگاه OVS باشد.

3. توابع ovs\_flow\_key\_extract, ovs\_ct\_fill\_key, ovs\_ct\_update\_key, و ovs\_lookup\_vport و ovs\_dp\_cmd\_get: این توابع زمان‌های اجرایی کمتری دارند، که نشان می‌دهد آنها به احتمال زیاد گلوگاه‌های عمده نیستند.
...
<p></p>
 ...
 در خروجی function\_call\_graph هم همین‌طور که انتظار می‌رود، این تابع بیشترین زمان اجرا را به خود اختصاص می‌دهد.
 <div dir="rtl" style: align="center">
<figure>
  <img src="https://github.com/matinborhani/ovs-tracing/blob/main/screenshots/Netcat%20Scenario/ftrace%20Netcat.png" alt="ftrace netcat " />
  <figcaption><em>تصویری از ftrace netcat</em></figcaption>
</figure>
</div>
<p></p>

زمانی که بین دو namespace تعریف شده در ovs ارتباط netcat برقرار می‌کنیم، بسته‌های شبکه باید از طریق bridge ovs عبور کنند. bridge ovs برای پیدا کردن مسیر بسته‌ها، از جدول flow استفاده می‌کند.
...
در سناریوی netcat نیز، گلوگاه ovs در تابع ovs_dp_process_packet قرار دارد که میانگین زمان پردازش آن 52.591 میکروثانیه است. این مقدار نسبتاً بالا است و می‌تواند باعث کاهش عملکرد OVS شود.
...
در نهایت در این قسمت به جمع بندی توابعی که نقش به سزایی در ایجاد گلوگاه در سوییچ مجازی OVS ایفا میکنند، پرداخته میشود.
 
 برای تحلیل گلوگاه‌ها در Open vSwitch (OVS) با توجه به توابع مذکور، می‌توانیم عملکرد OVS را از چند منظر مختلف بررسی کنیم:

1. تابع ovs_vport_receive  و ovs_vport_send و  ovs_dp_process_packet :
   - این توابع مربوط به دریافت، ارسال و پردازش بسته‌ها در سطح ورودی‌های مجازی (vports) هستند. 
   - گلوگاه احتمالی: در شرایط ترافیک بالا یا تعداد زیاد ورودی‌های مجازی، ممکن است شاهد کاهش عملکرد به دلیل بار پردازشی بالا باشیم.

2. تابع ovs_flow_key_extract و  ovs_flow_cmd_dump :
   - این توابع برای استخراج کلید‌های جریان، جستجو در جدول جریان‌ها، دامپ اطلاعات جریان‌ها و به‌روزرسانی آمار جریان‌ها استفاده می‌شوند.
   - گلوگاه احتمالی: پیچیدگی در مدیریت جدول‌های جریان، به‌ویژه در محیط‌هایی با تعداد زیاد جریان‌ها و قوانین. این امر می‌تواند بر عملکرد تأثیر بگذارد و زمان پردازش را افزایش دهد.

3. تابع ovs_dp_cmd_get:
   - این تابع برای دریافت اطلاعات مربوط به دیتاپس (datapath) در OVS استفاده می‌شود.
   - گلوگاه احتمالی: محدودیت‌ها و پیچیدگی‌های مربوط به مدیریت و نظارت بر دیتاپس‌ها، خصوصاً در مواقعی که تعداد دیتاپس‌ها و پیچیدگی شبکه افزایش می‌یابد.
